...
import {
  ...
  Settings, UserCog, Shield, LogOut,
} from 'lucide-react';
...
export default function SuperadminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#080808', fontFamily: 'Inter, system-ui, sans-serif' }}>
      {/* Sidebar */}
      <div style={{
        width: 220, background: '#0e0e0e', borderRight: '1px solid #1e1e1e',
        ...
        <div style={{ padding: '20px 16px 16px', borderBottom: '1px solid #1e1e1e' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              <p style={{ fontSize: 14, fontWeight: 700, color: '#ffffff', margin: 0 }}>iCube Admin</p>
              <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', margin: 0 }}>Superadmin Portal</p>
            </div>
          </div>
        </div>
        ...
        <div style={{ padding: '14px 16px', borderTop: '1px solid #1e1e1e' }}>
          {user && (
            <div style={{ marginBottom: 10 }}>
              <p style={{ fontSize: 13, fontWeight: 600, color: '#ffffff', margin: '0 0 2px' }}>{user.name}</p>
              <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', margin: 0, textTransform: 'capitalize' }}>{user.role}</p>
            </div>
          )}
