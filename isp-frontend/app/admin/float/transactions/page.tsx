...
import { api, FloatTransaction } from '@/lib/api';
...
// ── Operation badge ────────────────────────────────────────────────────────────
function OperationBadge({ op }: { op: 'credit' | 'debit' }) {
  if (op === 'credit')
    return <span style={{ background: 'rgba(29,158,117,0.15)', color: '#34d399', fontSize: 10, borderRadius: 9999, padding: '2px 8px', fontWeight: 600 }}>Credit</span>;
  return <span style={{ background: 'rgba(239,68,68,0.15)', color: '#f87171', fontSize: 10, borderRadius: 9999, padding: '2px 8px', fontWeight: 600 }}>Debit</span>;
}
...
// ── Transaction row ────────────────────────────────────────────────────────────
function TxRow({ row }: { row: FloatTransaction }) {
  const [hovered, setHovered] = useState(false);
  const isCredit = row.operation === 'credit';
  ...
  return (
    ...
    >
      ...
      <td style={{ padding: '10px 12px', fontSize: 11, color: '#888', maxWidth: 160 }}>
        <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'block' }}>
          {row.memo || '—'}
        </span>
      </td>
      <td style={{ padding: '10px 12px', fontSize: 12, fontWeight: 600, color: isCredit ? '#34d399' : '#f87171' }}>
        {isCredit ? '+' : '-'}{fmtUGX(row.amount)}
      </td>
      <td style={{ padding: '10px 12px', fontSize: 12, color: '#aaa' }}>{fmtUGX(row.balance_after)}</td>
    </tr>
  );
}
...
export default function FloatTransactionsPage() {
  const [data, setData]           = useState<FloatTransaction[]>([]);
  ...
  function load(p = page, pp = perPage, op = opFilter) {
    setLoading(true); setError('');
    api.float.transactions({ page: p, per_page: pp, operation: op !== 'all' ? op : undefined })
      .then(res => { setData(res.data); setTotal(res.total); })
      .catch((e: any) => setError(e.message))
      .finally(() => setLoading(false));
  }

  useEffect(() => { load(1, perPage, 'all'); }, []);

  function handleOpFilter(op: 'all' | 'credit' | 'debit') {
    setOpFilter(op);
    setPage(1);
    load(1, perPage, op);
  }
  ...
  return (
    <div className="min-h-screen" style={{ background: '#0f0f0f', padding: '24px 28px' }}>
      ...
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-bold text-white">Float Transactions</h1>
        <div className="flex items-center gap-2">
          <input type="date" value={fromDate} onChange={e => setFromDate(e.target.value)} style={inputStyle} />
          <input type="date" value={toDate} onChange={e => setToDate(e.target.value)} style={inputStyle} />
          <button onClick={() => { setPage(1); load(1, perPage, opFilter); }}
      ...
      <div className="flex gap-2 mb-4">
        ...
        ] as { key: 'all' | 'credit' | 'debit'; label: string }[]).map(({ key, label }) => (
          <button
            key={key}
            onClick={() => handleOpFilter(key)}
            style={{
              background: opFilter === key ? 'rgba(29,158,117,0.2)' : '#1a1a1a',
              color: opFilter === key ? '#1D9E75' : '#666',
              border: `1px solid ${opFilter === key ? 'rgba(29,158,117,0.3)' : '#2a2a2a'}`,
      ...
      <div style={{ background: '#1a1a1a', border: '1px solid #222', borderRadius: 12, overflow: 'hidden' }}>
        {loading ? (
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {data.length === 0 ? (
                  <tr><td colSpan={9} style={{ textAlign: 'center', padding: '32px', color: '#555', fontSize: 13 }}>No transactions found</td></tr>
                ) : (
                  data.map(row => <TxRow key={row.id} row={row} />)
                )}
              </tbody>
            </table>
          </div>
        ...
        <Pagination
          ...
          onPage={p => { setPage(p); load(p, perPage, opFilter); }}
          onPerPage={n => { setPerPage(n); setPage(1); load(1, n, opFilter); }}
        />
      </div>
    </div>
  );
